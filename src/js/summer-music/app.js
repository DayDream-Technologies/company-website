/**
 * GR Summer Music - Main Application
 *
 * Structurally identical to src/js/ada-events/app.js; differs only in the
 * data file path and the localStorage keys so this page keeps its own cache
 * and user preferences separate from the GR and Ada pages.
 */
...
(function() {
  ...
  const DATA_URL = 'src/data/summer-music-events.json';
  const CACHE_KEY = 'summer-music-cache';
  const PREFS_KEY = 'summer-music-prefs';
  ...
  async function loadEvents() {
    ...
    try {
      ...
      updateUI();
      showStatus('error', 'No events data found. Run the scrape:summer-music script to generate data.');
    }
  }
